...
subList  <-  data.frame(SUB_ID=rep(subjectList$SUB_ID[1],9), l=1:9, r=1:9, tract=1:9)
...
for (i in 1:nrow(subjectList)){
	...
	j <- i +1
	subListTemp <- data.frame(SUB_ID=rep(subjectList$SUB_ID[j],9), l=1:9, r=1:9, tract=1:9)
	subList <- rbind(subList, subListTemp)
}

allTracts_FA <-(subList[1:(nrow(subList)-9),])
...
allTracts_MD <-(subList[1:(nrow(subList)-9),])
...
allTracts_AD <-(subList[1:(nrow(subList)-9),])
...
allTracts_RD <-(subList[1:(nrow(subList)-9),])
